...
function parseTimestamps(events: TraceEvent[]): StepTiming[] {
  ...
  for (const [step, t] of [...byStep.entries()].sort((a, b) => a[0] - b[0])) {
    ...
    timings.push({
      step,
      thinkSec: thinkStart && thinkEnd ? Math.max(0, (thinkEnd - thinkStart) / 1000) : 0,
      actSec: actStart && actEnd ? Math.max(0, (actEnd - actStart) / 1000) : 0,
    });
  }
  return timings;
}
...
export function StatsView({ events }: { events: TraceEvent[] }) {
  ...
  const totalThink = timings.reduce((s, t) => s + t.thinkSec, 0);
  const totalAct = timings.reduce((s, t) => s + t.actSec, 0);
  ...
  return (
    <div style={{ padding: '8px 0' }}>
      <Row gutter={16} style={{ marginBottom: 24 }}>
        <Col span={6}>
          <Statistic title={t('stats.totalThink')} value={totalThink} suffix="s" precision={1} />
        </Col>
        <Col span={6}>
          <Statistic title={t('stats.totalAct')} value={totalAct} suffix="s" precision={1} />
        </Col>
        <Col span={6}>
          <Statistic
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={timings}>
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="thinkSec" name="Think (s)" fill={palette.chartSeries[0]} stackId="a" />
              <Bar dataKey="actSec" name="Act (s)" fill={palette.chartSeries[1]} stackId="a" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
