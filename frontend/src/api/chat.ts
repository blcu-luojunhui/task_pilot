...
export interface GeneratePrdFromMessagesResponse {
  prd: string;
}

export async function generatePrdFromMessages(
  conversationId: string,
  messageIds: number[],
): Promise<GeneratePrdFromMessagesResponse> {
  return unwrap(
    apiClient.post<{ data: GeneratePrdFromMessagesResponse }>(
      `/chat/conversations/${encodeURIComponent(conversationId)}/prd_from_messages`,
